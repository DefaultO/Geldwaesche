...
I put a comment saying "Geldwäsche" next to most things I edited or added ontop of the [base](https://github.com/appsec-labs/Advanced_Packet_Editor). This got never sold, but had a loader at some point as an anti-leak measure. All of that got removed, so I hadn't to include the projects I used using project references. The Core of this project is in ``Main.cs``, and the exploit / trigger packet detection happens at the bottom of the ``UpdateMainGrid(byte[] data)`` function, [here](https://github.com/DefaultO/Geldwaesche/blob/8f0a62aa94373925aea0c3b5891e0b36058ad579/src/PacketEditor/Main.cs#L1012):
```csharp
// Geldwäsche
