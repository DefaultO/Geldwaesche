...
DATA_TO_SEND";
        ...
        void ForceDisconnect(string forceDisconnectPacket)
        {
            strPipeMsgOut.sockid = strPipeMsgIn.sockid;
            strPipeMsgOut.command = Glob.CMD_INJECT;
            strPipeMsgOut.function = Glob.FUNC_SEND;
            strPipeMsgOut.datasize = ae.GetBytes(forceDisconnectPacket).Length;

            for (int count = 0; count < 4; count++)
            {
                pipeOut.Write(Glob.RawSerializeEx(strPipeMsgOut), 0, Marshal.SizeOf(strPipeMsgOut));
                pipeOut.Write(ae.GetBytes(forceDisconnectPacket), 0, strPipeMsgOut.datasize);
            }
        ...
        PipeLoop:
            ...
            Invoke(delExitProc);
            /* Old reattach logic that doesn't even work. Not mine.
        ...
        private void frmMain_Load(object sender, EventArgs e)
        {
            ...
            ToggleFeatures tglForm = new ToggleFeatures();
            tglForm.Show(); // Geldwäsche

            // The better way, of using 'Path.Combine()' for whatever reason only returns "\Accounts.cfg"
            string path = System.IO.Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location) + @"\Accounts.cfg";
            if (File.Exists(path))
            {
                string[] accountConfiguration = File.ReadAllLines(path); int index = 0;
                foreach (string line in accountConfiguration)
                {
                    // Ignore Comments, yes you can put comments into the Config File. Why not.
                    if (line.StartsWith("//") || line.StartsWith(Environment.NewLine) || String.IsNullOrEmpty(line)) continue;
                    Geldwäsche.Accounts[index] = line.Split('=')[1];
                    index++;
                }
            }
            else
            {
                FileStream fileHandle = File.Create(path); fileHandle.Flush(); fileHandle.Close();
                using (StreamWriter sw = new StreamWriter(path))
                {
                    sw.Write($"// Copy the login packet, by finding it in the list and right-clicking it. Copy > ASCII will result in you having something like this in your clipboard:{Environment.NewLine}// %xt%MafiaEx3%lgn%2%jikkfdgdmgfdfgk%uhaiodfhgiuhadf%de%199%196%1619269605256306297%\\0{Environment.NewLine}// A Login packet will have %xt%MafiaEx3%lgn at the start of it. Notice the lgn at the end, which is short for login.{Environment.NewLine}// The Server is what stands behind the second %, this can vary depending on what Server you play on. Put the whole copied thing after the correct = below.{ Environment.NewLine}{ Environment.NewLine}MafiaEx3={Environment.NewLine}MafiaEx4={Environment.NewLine}MafiaEx_9={Environment.NewLine}MafiaEx_8={Environment.NewLine}MafiaEx_10={Environment.NewLine}MafiaEx5={Environment.NewLine}MafiaEx_11={Environment.NewLine}MafiaEx={Environment.NewLine}MafiaEx2={Environment.NewLine}MafiaEx_6={Environment.NewLine}MafiaEx_7="); sw.Flush(); sw.Close();
                }
                MessageBox.Show($"We created a new File here, in which you put the logged login packets of alt accounts (as ASCII text - not bytes). These Accounts should be trash accounts. You can give them random names and a random password. This file only gets used to force a disconnect. Your put in data will not get used in any other way. You can find it here:{Environment.NewLine}{Environment.NewLine}{path}{Environment.NewLine}{Environment.NewLine}After you click 'OK', this MessageBox will disappear and the whole application will close. Make sure you go add these accounts now before you forget. Without a set Account for your server, the force disconnect exploits won't work, as you provided the program with no Account to log-into.", "Geldwäsche: Add your Accounts now", MessageBoxButtons.OK, MessageBoxIcon.Information);
                Environment.Exit(0);
            }
        }
...
" + data;
        ...
        private void checkForGGG_Tick(object sender, EventArgs e)
        {
            if (!attached)
            {
                System.Diagnostics.Process process;
                if (GetProcessesByName("Goodgame Gangster", out process))
                {
                    processID = process.Id;
                    if (!Attach(processID, process.MainModule.FileName))
                    {
                        mnuFileDetach.Enabled = false;
                        reAttachToolStripMenuItem.Enabled = true;
                    }
